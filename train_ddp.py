...
def prepare_loaders(df, fold, debug=False):
    ...
    train_sampler = DistributedSampler(train_dataset, shuffle=True, num_replicas=2, rank=CFG.local_rank)
    valid_sampler = DistributedSampler(valid_dataset, shuffle=False, num_replicas=2, rank=CFG.local_rank)

    train_loader = DataLoader(train_dataset, batch_size=CFG.train_bs if not debug else 20,
                              num_workers=4, sampler=train_sampler, pin_memory=True, drop_last=False)
    valid_loader = DataLoader(valid_dataset, batch_size=CFG.valid_bs if not debug else 20,
                              num_workers=4, sampler=valid_sampler, pin_memory=True)
...
def main(rank, world_size):
    ...
    def mask_path(ID):
        return f"{BASE}/train_masks/{ID}.png"
